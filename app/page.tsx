...
function mobileDuration(act: Act) {
  const start = minutes(act.start);
  const end = minutes(act.end);
  if (start === null || end === null) return 92;
  return Math.max(76, (end - start) * MOBILE_PIXELS_PER_MINUTE);
}

const PIXELS_PER_MINUTE = 1.8;
const MOBILE_PIXELS_PER_MINUTE = 0.92;
const HOUR = 60;
...
export default function Home() {
  ...
  const mobileTimelineWidth = Math.max(720, (timelineEnd - timelineStart) * MOBILE_PIXELS_PER_MINUTE);
                    ...
                    {stages.map((stage) => (
                      <section key={stage} className="grid min-h-[122px] rounded-[8px] border border-white/10 bg-[rgba(15,23,27,0.86)]" style={{ gridTemplateColumns: "78px 1fr" }}>
                        ...
                        <div className="relative min-h-[122px] overflow-hidden">
                          ...
                          {nowTop !== null && currentMinutes !== null && (
                            <div className="pointer-events-none absolute bottom-0 top-0 z-20 border-l-2 border-red-500 shadow-[0_0_14px_rgba(239,68,68,0.75)]" style={{ left: (currentMinutes - timelineStart) * MOBILE_PIXELS_PER_MINUTE }} />
                          )}
                            ...
                            return (
                              ...
                              >
                                <span className="flex w-full items-center justify-between gap-1 text-[10px] font-bold uppercase text-zinc-400">
                                  <span className="truncate">{act.start}-{act.end}</span>
                                  {selected && <Check size={14} className="shrink-0 text-green-300" />}
                                </span>
